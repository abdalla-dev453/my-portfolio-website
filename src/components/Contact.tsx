...
function Field({ label, id, type = 'text', value, error, onChange }: FieldProps) { return <label className="mt-8 block"><span className="text-[10px] font-bold uppercase tracking-[0.18em] text-stone-500">{label}</span><input id={id} required type={type} value={value} aria-invalid={Boolean(error)} aria-describedby={error ? `${id}-error` : undefined} onChange={onChange} className="mt-3 w-full border-b border-line bg-transparent pb-3 text-sm text-paper outline-none placeholder:text-stone-600 focus:border-accent focus-visible:ring-2 focus-visible:ring-accent/40" placeholder={label === 'Name' ? 'Your name' : label === 'Email' ? 'you@company.com' : 'A short introduction'} />{error && <p id={`${id}-error`} className="mt-2 text-xs text-red-400">{error}</p>}</label>; }
